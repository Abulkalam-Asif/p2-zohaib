...
const AppsTabsTable = ({ appsData, selectedTab }) => {
  return (
    <>
      <div className="relative overflow-x-auto  md:max-w-[700px] sm:max-w-[600px] xs:max-w-[350px]">
        <table className="w-full text-base text-left text-gray-500 min-w-[500px]">
          <thead className="text-sm text-zinc-900 uppercase bg-themeColor0Lighter">
            <tr className='grid grid-cols-4 md:grid-cols-3'>
              <th scope="col" className="px-6 py-3 col-span-2 font-medium md:col-span-1">
                Apps
              </th>
          ...
          <tbody>
            {appsData
              ...
              .map((app, index) => (
                <tr key={index} className="grid  grid-cols-4 bg-white border-b-2 border-b-themeColor0Lighter md:grid-cols-3">
                  <td
                    className="col-span-2 flex items-center gap-x-2 px-6 py-4 text-sm font-semibold text-black whitespace-nowrap md:col-span-1"
                  >
